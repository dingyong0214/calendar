		...
		methods: {
			...
			btnFix(show) {
				...
				if (this.type == 1) {
					...
					const _lunar = new Lunar(new Date(`${year}/${month}/${day} 23:59`))
					let lunar = calendar.solar2lunar(year, month, day);
					if (_lunar) {
						lunar.gzYear = _lunar.gz[0]
						lunar.gzMonth = _lunar.gz[1]
						lunar.gzDay = _lunar.gz[2]
					}
					this.$emit('change', {
						year: year,
						month: month,
					...
					const _slunar = new Lunar(new Date(`${year}/${month}/${day} 23:59`))
					if (_slunar) {
						startLunar.gzYear = _slunar.gz[0]
						startLunar.gzMonth = _slunar.gz[1]
						startLunar.gzDay = _slunar.gz[2]
					}
					...
					const _elunar = new Lunar(new Date(`${year}/${month}/${day} 23:59`))
					if (_elunar) {
						endLunar.gzYear = _elunar.gz[0]
						endLunar.gzMonth = _elunar.gz[1]
						endLunar.gzDay = _elunar.gz[2]
					}
					this.$emit('change', {
